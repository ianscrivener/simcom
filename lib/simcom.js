(function() {
  ...
  SimCom = (function() {
    ...
    SimCom.prototype.execute = function(command) {
      var args, callback, response, timeout;
      if (!command) {
        return;
      }
      ...
      timeout = args.length > 1 && typeof args.slice(-1)[0] === 'number' ? args.pop() : null;
      return this.modem.execute(command, timeout, response, pdu, callback);
    };
    ...
    Object.keys(simple_methods).forEach(function(name) {
      SimCom.prototype[name] = function(timeout) {
        var args, callback, defer, response;
        args = [].slice.apply(arguments);
        callback = args.length > 1 && typeof args.slice(-1)[0] === 'function' ? args.pop() : null;
        pdu = args.length > 1 && typeof args.slice(-1)[0] === 'boolean' ? args.pop() : null;
        response = args.length > 1 && typeof args.slice(-1)[0] === 'string' ? args.pop() : null;
        defer = Q.defer();
        this.execute(simple_methods[name], timeout, response, pdu, callback).then(function(res) {
