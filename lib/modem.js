(function() {
  ...
  Modem = (function() {
    ...
    Modem.prototype.execute = function() {
      var args, callback, command, defer, pdu, ref, response, timeout;
      command = arguments[0], args = 2 <= arguments.length ? slice.call(arguments, 1) : [];
      if (command == null) {
        return;
      }
      callback = typeof args.slice(-1)[0] === 'function' ? args.pop() : null;
      pdu = (ref = typeof args.slice(-1)[0]) === 'boolean' || ref === 'object' ? args.pop() : false;
      ...
      log && log.write(JSON.stringify(defer.execution) + "\n");
    ...
    fetchExecution = function() {
      var cmd, defer, execution, ref;
      defer = this.executions[0];
      if (!defer) {
        return;
      }
      execution = defer.execution;
      cmd = (ref = execution.exec) != null ? ref.split("\r", 1).shift() : void 0;
      this.write(cmd + "\r");
    ...
    isResultCode = function(line) {
      return /(^OK|ERROR|BUSY|DATA|NO ANSWER|NO CARRIER|NO DIALTONE|OPERATION NOT ALLOWED|COMMAND NOT SUPPORT|\+CM[EPS]*|> $)|(^CONNECT( .+)*$)/i.test(line);
    };

    isErrorCode = function(line) {
      return /^(\+CM[EPS]\s)?ERROR(\:.*)?|BUSY|NO ANSWER|NO CARRIER|NO DIALTONE|OPERATION NOT ALLOWED|COMMAND NOT SUPPORT$/i.test(line);
    };
    ...
    processResponse = function() {
      var cmd, defer, error, exec, execution, m, ref, response, responseCode;
      responseCode = this.lines.pop();
      defer = this.executions[0];
      execution = defer && defer.execution;
      exec = (execution != null ? (ref = execution.exec) != null ? ref.split("\r") : void 0 : void 0) || "";
      cmd = exec.shift();
      if (responseCode === "> ") {
