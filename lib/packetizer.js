(function() {
  var EventEmitter, Packetizer, checkEnd, decode, isResultCode, util,
  ...
  Packetizer = (function() {
    ...
    Packetizer.prototype.packetize = function() {
      ...
      this.uart.on("data", function(bytes) {
        var data, i, line, packet, thing, _i, _len;
        i = 0;
        data = decode(bytes).replace(new RegExp("" + self.ender + "+", 'g'), "\n").replace(new RegExp("^" + self.ender + "|" + self.ender + "$", 'g'), "").split(self.ender);
        packet = [];
        for (_i = 0, _len = data.length; _i < _len; _i++) {
          line = data[_i];
          console.log("line", line, isResultCode(line));
          if (!self.checkBlacklist(line)) {
